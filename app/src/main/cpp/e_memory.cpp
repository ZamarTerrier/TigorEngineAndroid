...
void *MakeAlloc(AllocObj *obj){
    ...
    if(alloc_memory_head->node == NULL){
        alloc_memory_head->next = (struct ChildStack *)calloc(1, sizeof(ChildStack));
        ...
        child->next = (struct ChildStack *)calloc(1, sizeof(ChildStack));
        child->node = obj;
    }
...
void *AllocateMemoryN(int32_t count, int32_t size, char *name){
    ...
    AllocObj *obj = (AllocObj *)calloc(1, sizeof(AllocObj));
    obj->size = size;
    obj->count = count;

    uint32_t len = strlen(name);
    obj->name = (char *)calloc(len + 1, sizeof(char));
